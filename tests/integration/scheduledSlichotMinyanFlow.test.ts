...
describe("scheduled slichot minyan flow", () => {
  beforeAll(async () => {
...
בכדי להתעדכן במצב המניין, או לעדכן את הנוכחות בכל שלב - פשוט תכתוב *עדכון*`
  ...
  it("minyan schedule executed again after 20 minutes -> only snoozed notifications are sent", async () => {
    await scheduleExecution("04:50");
    await expectTzenterTextMessage(
      user2.phoneNum,
      `זוהי תזכורת לתפילת סליחות בשעה 05:30 במניין איצקוביץ
...
האם תגיע?`
  ...
  it("minyan schedule executed again 1 minute before due time -> last execution -> schedule is triggering all user notifications", async () => {
    await scheduleExecution("05:29");
    await expectTzenterTextMessageSequence([
      {
        phoneNum: user2.phoneNum,
        message: `זוהי תזכורת לתפילת סליחות בשעה 05:30 במניין איצקוביץ

האם תגיע?`,
      },
      {
        phoneNum: user1.phoneNum,
        message: `עדכון לתפילת סליחות בשעה 05:30 במניין איצקוביץ
...
המתפללים הבאים אשרו הגעה לתפילת סליחות במניין איצקוביץ בשעה 05:30
